...
const multipleSelection = ref<Attendance[]>([])
const handleSelectionChange = (attendances: Attendance[]) =>
  (multipleSelection.value = attendances)

// 单个删除
const handleDelete = async ({ attendance_id }: Attendance) => {
  if (attendance_id) {
    const { data } = await delAttendance(attendance_id)
    if (data.code === 200) {
      await getAttendanceListPage()
...
/* 增加 编辑相关 */
const dialogForm = ref<Attendance>({})
...
// 设置dialog
const setDialog = async (operate: string, row?: Attendance) => {
  if (operate === 'insert') {
    dialog.title = '新增学院'
    multipleSelection.value = []
  }
  if (operate === 'update') {
    if (row) {
      ...
      dialogForm.value = cloneDeep(multipleSelection.value[0] as Attendance)
    }
    dialog.title = '编辑学院'
  }
  dialog.show = true
  ...
  await formEl.validate(async (valid) => {
    if (valid) {
      if (dialog.operate === 'insert') {
        const { data } = await addAttendance(dialogForm.value)
        if (data.code === 200) {
          await getAttendanceListPage()
          dialog.show = false
      ...
      if (dialog.operate === 'update') {
        const { data } = await updateAttendance(dialogForm.value)
        if (data.code === 200) {
          await getAttendanceListPage()
          dialog.show = false
